...
public class SerializableAggregateListProjectorTests
{
    ...
    public async Task SerializeDeserialize_BranchListProjector_Success()
    {
        ...
        // Act
        var serializedJson
            = await SerializableAggregateListProjector.SerializeAggregateList(originalProjector, _domainTypes);
    ...
    public async Task SerializeDeserialize_MixedTypeListProjector_Success()
    {
        ...
        var emptyAggregate
            = originalProjector.Aggregates.Values.FirstOrDefault(a => a.GetPayload() is EmptyAggregatePayload);
    ...
    {
        var originalProjector = CreateLargeBranchListProjector();
    ...
    {
        var emptyProjector
            = new AggregateListProjector<BranchProjector>(ImmutableDictionary<PartitionKeys, Aggregate>.Empty);

        // Act
        var serializedJson
            = await SerializableAggregateListProjector.SerializeAggregateList(emptyProjector, _domainTypes);
        var serializedString = serializedJson.IsSuccess ? serializedJson.GetValue() : null;
        Assert.NotNull(serializedString);
