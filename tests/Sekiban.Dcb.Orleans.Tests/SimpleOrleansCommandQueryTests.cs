...
public class SimpleOrleansCommandQueryTests : IAsyncLifetime
{
    ...
    public async Task Should_Execute_Command_Through_Orleans()
    {
        ...
        if (_executor is null)
        {
            throw new InvalidOperationException(
                $"_executor not initialized; domainTypes null={_domainTypes is null}, eventStore null={_eventStore is null}");
        }
        // Arrange
    ...
    public async Task Orleans_PubSub_Should_Work_With_EventPublisher_And_Direct_Stream()
    {
        ...
        Assert.Contains(receivedPayloads, p => p is TestEntityCreatedEvent created && created.Name == "Event1");
        Assert.Contains(receivedPayloads, p => p is TestEntityUpdatedEvent updated && updated.Name == "Event2");
    ...
    private async Task<ResultBox<TagState>> WaitForTagVersionAsync(TagStateId id, int minVersion, TimeSpan timeout)
    {
        ...
        var last = ResultBox.Error<TagState>(new InvalidOperationException("not started"));
        var attempts = 0;
        ...
        Console.WriteLine(
            $"[WaitForTagVersion] Timeout after {attempts} attempts. Last version was {(last.IsSuccess ? last.GetValue().Version.ToString() : "error")}");
        return last;
    }
