...
public class ClientSpec : AggregateTest<Client, FeatureCheckDependency>
{
    ...
    public void ClientCreateSpec()
    {
        ...
        ThenStateIs(
            ...
            {
                AggregateId = GetAggregateId(), Version = GetCurrentVersion(),
    ...
    public void EnvironmentChangeCommandTest()
    {
        ...
        RunEnvironmentCommand(
            new CreateClient { BranchId = branchId, ClientName = "NameFirst", ClientEmail = "test@example.com" },
            otherClientId);
        ...
        RunEnvironmentCommand(
            new UseLoyaltyPoint(otherClientId, DateTime.Today, LoyaltyPointUsageTypeKeys.TravelCarRental, 30, "test"));
    }
    ...
    {
        var branchId = RunEnvironmentCommand(new CreateBranch("TEST"));
        WhenCommandWithPublishAndBlockingSubscriber(
            new CreateClientWithBranchSubscriber(branchId, "client", "client@example.com"));
    ...
    public async Task CanBlockNonBlockingSubscriberFunc()
    {
        var branchId = RunEnvironmentCommand(new CreateBranch("TEST"));
        WhenCommandWithPublishAndBlockingSubscriber(
            _ => new CreateClientWithBranchSubscriber(branchId, "client", "client@example.com"));
        var branch = GetEnvironmentAggregateState<Branch>(branchId);
        Assert.Equal(1, branch.Payload.NumberOfMembers);
        await Task.CompletedTask;
    }
