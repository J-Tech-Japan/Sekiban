...
public class GeneralQuerySampleSpec : UnifiedTest<FeatureCheckDependency>
{
    ...
    public void GeneralQueryBasicTest()
    {
        ...
        ResultBox.FromValue(GetQueryResponse(new GeneralListQuerySample.Parameter("test")))
            .Combine(ResultBox.FromValue(GetQueryResponse(new GeneralListQuerySampleNext("test"))))
            .Scan(Assert.Equal)
            .Conveyor(
                _ => ResultBox.FromValue(GetQueryResponse(new GeneralListQuerySample.Parameter("example.com")))
