...
public class GeneralSekibanExecutorDomainTests
{
    ...
    public async Task EnrollStudent_Should_Succeed_When_Both_Exist_And_Have_Capacity()
    {
        ...
        var result = await _commandExecutor.ExecuteAsync(enrollCommand, EnrollStudentInClassRoomHandler.HandleAsync);
    ...
    public async Task EnrollStudent_Should_Fail_When_Student_At_Max_Capacity()
    {
        ...
        await _commandExecutor.ExecuteAsync(new CreateStudent(studentId, "John Doe", 2));
        ...
        for (var i = 0; i < 2; i++)
        {
            ...
            await _commandExecutor.ExecuteAsync(
                new CreateClassRoom(classRoomId, $"Class {i}"),
                CreateClassRoomHandler.HandleAsync);
        ...
        await _commandExecutor.ExecuteAsync(
            new CreateClassRoom(thirdClassRoomId, "Class 3"),
            CreateClassRoomHandler.HandleAsync);
    ...
    public async Task EnrollStudent_Should_Fail_When_ClassRoom_Is_Full()
    {
        ...
        for (var i = 0; i < 2; i++)
        {
            ...
            await _commandExecutor.ExecuteAsync(new CreateStudent(studentId, $"Student {i}"));
        ...
        await _commandExecutor.ExecuteAsync(new CreateStudent(thirdStudentId, "Student 3"));
    ...
    public async Task Complex_Scenario_Multiple_Students_And_ClassRooms()
    {
        ...
        await _commandExecutor.ExecuteAsync(
            new EnrollStudentInClassRoom(student1, classRoom1),
            EnrollStudentInClassRoomHandler.HandleAsync);
        await _commandExecutor.ExecuteAsync(
            new EnrollStudentInClassRoom(student1, classRoom2),
            EnrollStudentInClassRoomHandler.HandleAsync);
        await _commandExecutor.ExecuteAsync(
            new EnrollStudentInClassRoom(student2, classRoom1),
            EnrollStudentInClassRoomHandler.HandleAsync);
        await _commandExecutor.ExecuteAsync(
            new EnrollStudentInClassRoom(student3, classRoom2),
            EnrollStudentInClassRoomHandler.HandleAsync);
    ...
    public async Task Concurrent_Commands_Should_Be_Handled_Correctly()
    {
        ...
        // Create all students
        var createTasks = studentIds
            .Select(id => _commandExecutor.ExecuteAsync(new CreateStudent(id, $"Student {id}")))
            .ToList();
        ...
        var enrollTasks = studentIds
            .Select(id => _commandExecutor.ExecuteAsync(
