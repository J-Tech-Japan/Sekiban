...
public class ConsistencyReservationRulesTest
{
    ...
    public async Task NonConsistencyTag_Should_Not_Create_TagState()
    {
        ...
        var result = await _executor.ExecuteAsync(new SimpleCommand(), (cmd, ctx) =>
            Task.FromResult(EventOrNone.Event(new DummyEvent("A"), nonConsistency)));
    ...
    public async Task ConsistencyTag_With_Explicit_SortableUniqueId_Should_Use_It()
    {
        ...
        var result = await _executor.ExecuteAsync(new SimpleCommand(), (cmd, ctx) =>
            Task.FromResult(EventOrNone.Event(new DummyEvent("B"), consistencyTag)));
        Assert.True(result.IsSuccess);
    }
    ...
    public async Task ConsistencyTag_Without_SortableUniqueId_Should_Use_Accessed_State()
    {
        ...
        var first = await _executor.ExecuteAsync(new SimpleCommand(), (cmd, ctx) =>
            Task.FromResult(EventOrNone.Event(new DummyEvent("C1"), consistencyTag)));
        Assert.True(first.IsSuccess);

        // Second write should access state and use LastSortableUniqueId
        // Access state beforehand to ensure it is tracked
        await _executor.ExecuteAsync(new SimpleCommand(), async (cmd, ctx) =>
        {
            await ctx.GetStateAsync<DummyProjector>(baseTag);
            return EventOrNone.None; // just for tracking
        });
        var second = await _executor.ExecuteAsync(new SimpleCommand(), (cmd, ctx) =>
            Task.FromResult(EventOrNone.Event(new DummyEvent("C2"), consistencyTag)));
