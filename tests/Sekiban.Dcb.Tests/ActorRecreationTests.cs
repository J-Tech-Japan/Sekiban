...
public class ActorRecreationTests
{
    ...
    public async Task TagStateActor_With_TagConsistentActor_Should_Both_CatchUp()
    {
        ...
        var consistentActor1
            = (await _accessor.GetActorAsync<ITagConsistentActorCommon>(tagConsistentActorId)).GetValue();
        ...
        // Get state - should include both events after catch up
        var state2 = await stateActor2.GetStateAsync();
        Assert.Equal(2, state2.Version); // Should include both events
        Assert.Equal(event2.SortableUniqueIdValue, state2.LastSortedUniqueId);
    }
}
