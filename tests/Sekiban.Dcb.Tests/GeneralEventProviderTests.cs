...
public class GeneralEventProviderTests
{
    ...
    private Event CreateTestEvent(string eventType, DateTime timestamp, string? data = null)
    {
    ...
    public async Task EventsAreProvidedInOrder()
    {
        ...
        // Act
        var handle = await provider.StartAsync(async (evt, isSafe) =>
        {
        ...
        for (var i = 1; i < receivedEvents.Count; i++)
        {
            Assert.True(
                string.Compare(
    ...
    public async Task StatisticsAreTrackedCorrectly()
    {
        ...
        // Act
        var handle = await provider.StartAsync(async (evt, isSafe) => await Task.CompletedTask);
