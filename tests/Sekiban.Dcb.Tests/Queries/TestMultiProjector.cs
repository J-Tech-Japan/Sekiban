...
public record TestMultiProjector : IMultiProjector<TestMultiProjector>
{
    ...
    public static ResultBox<TestMultiProjector> Project(TestMultiProjector payload, Event ev, List<ITag> tags)
    {
        ...
        {
            ItemAdded itemAdded => ResultBox.FromValue(payload with
            {
                Items = payload.Items.Append(new TestItem(
                    itemAdded.Id,
                    itemAdded.Name,
                    itemAdded.Category,
                    itemAdded.Price,
                    itemAdded.CreatedAt)).ToList(),
                TotalCount = payload.TotalCount + 1
            }),
            ItemRemoved itemRemoved => ResultBox.FromValue(payload with
            {
...
public record ItemAdded(
    ...
    string Category,
    decimal Price,
    DateTime CreatedAt) : IEventPayload;
