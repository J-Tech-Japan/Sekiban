...
public class GeneralQueryExecutorTestsV2
{
    ...
    private TestMultiProjector CreateSampleProjector()
    {
        ...
        {
            (CreateEvent(new ItemAdded(Guid.NewGuid(), "Item 1", "Electronics", 100m, DateTime.UtcNow.AddDays(-5))),
                new List<ITag>()),
            (CreateEvent(new ItemAdded(Guid.NewGuid(), "Item 2", "Books", 25m, DateTime.UtcNow.AddDays(-4))),
                new List<ITag>()),
            (CreateEvent(new ItemAdded(Guid.NewGuid(), "Item 3", "Electronics", 250m, DateTime.UtcNow.AddDays(-3))),
                new List<ITag>()),
            (CreateEvent(new ItemAdded(Guid.NewGuid(), "Item 4", "Books", 15m, DateTime.UtcNow.AddDays(-2))),
                new List<ITag>()),
            (CreateEvent(new ItemAdded(Guid.NewGuid(), "Item 5", "Clothing", 50m, DateTime.UtcNow.AddDays(-1))),
                new List<ITag>())
        };
    ...
    private Event CreateEvent(IEventPayload payload)
    {
        ...
        return new Event(payload, sortableId, payload.GetType().Name, eventId, metadata, new List<string>());
    }
    ...
    public async Task ExecuteListQueryAsync_FilterByCategory_ReturnsOnlyMatchingItems()
    {
        ...
        // Act
        var result
            = await _queryExecutor.ExecuteListQueryAsync<TestMultiProjector, GetItemsByCategoryQueryV2, TestItem>(
                query,
                () => Task.FromResult(ResultBox.FromValue(projector)));
    ...
    public async Task ExecuteWithHandlerAsync_CustomHandler_ReturnsCorrectResult()
    {
        ...
        // Act
        var result = await _queryExecutor.ExecuteWithHandlerAsync(
            () => Task.FromResult(ResultBox.FromValue(projector)),
