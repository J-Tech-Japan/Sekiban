...
{
    public class ValueObjectConvertibleAttribute<TValueObject> : ValidationAttribute
        where TValueObject : class, ISingleValueObjectBase
    {
        protected override ValidationResult? IsValid(object? value, ValidationContext validationContext)
        {
            ...
            try
            {
                dynamic? valueObjectInstance =
                    Activator.CreateInstance(typeof(TValueObject), valueObject) as TValueObject;
                return valueObjectInstance?.Value?.GetType() == valueType ? ValidationResult.Success
                    : new ValidationResult("値の形式が正しくありません。");
            }
            catch (Exception ex)
            {
