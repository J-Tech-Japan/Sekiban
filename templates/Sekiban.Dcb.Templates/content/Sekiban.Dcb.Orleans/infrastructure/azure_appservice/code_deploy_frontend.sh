...
FRONTEND_PATH=$(jq -r '.frontendRelativePath' "$CONFIG_PATH")
...
echo "Building and publishing .NET application (self-contained for linux-x64)..."
pushd "$FRONTEND_PATH"
...
dotnet publish -c Release -o "$PUBLISH_DIR" --self-contained true -r linux-x64
