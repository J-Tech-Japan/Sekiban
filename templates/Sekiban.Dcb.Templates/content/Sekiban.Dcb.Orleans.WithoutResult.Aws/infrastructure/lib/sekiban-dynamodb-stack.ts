...
export class SekibanDynamoDbStack extends cdk.Stack {
  constructor(scope: Construct, id: string, props: SekibanDynamoDbStackProps) {
    ...
    const rdsSecret = new secretsmanager.Secret(this, 'RdsSecret', {
      secretName: `sekibandcborleansaws-${envName}-rds`,
    ...
    // ========================================
    const apiEcrRepoName = `sekibandcborleansaws-api-${envName}`;
    const webEcrRepoName = `sekibandcborleansaws-web-${envName}`;
    ...
    const cluster = new ecs.Cluster(this, 'Cluster', {
      vpc,
      clusterName: `sekibandcborleansaws-${envName}`,
      containerInsights: true,
    });

    // Cloud Map namespace for service discovery
    const namespace = new servicediscovery.PrivateDnsNamespace(this, 'Namespace', {
      name: `sekibandcborleansaws-${envName}.internal`,
      vpc,
    });
    ...
    const apiLogGroup = new logs.LogGroup(this, 'ApiLogGroup', {
      logGroupName: `/ecs/sekibandcborleansaws-api-${envName}`,
      retention: logs.RetentionDays.ONE_WEEK,
      removalPolicy: cdk.RemovalPolicy.DESTROY,
    });

    const webLogGroup = new logs.LogGroup(this, 'WebLogGroup', {
      logGroupName: `/ecs/sekibandcborleansaws-web-${envName}`,
      retention: logs.RetentionDays.ONE_WEEK,
      removalPolicy: cdk.RemovalPolicy.DESTROY,
    });
    ...
    const apiServiceUrl = `http://apiservice.sekibandcborleansaws-${envName}.internal:${config.orleans.httpPort}`;
    ...
    // Access via Cloud Map: apiservice.sekibandcborleansaws-{env}.internal
    ...
    const distribution = new cloudfront.Distribution(this, 'Distribution', {
      ...
      comment: `SekibanDcbOrleansAws ${envName} - CloudFront Distribution`,
    });
    ...
    new cdk.CfnOutput(this, 'InternalApiEndpoint', {
      value: `http://apiservice.sekibandcborleansaws-${envName}.internal:${config.orleans.httpPort}`,
