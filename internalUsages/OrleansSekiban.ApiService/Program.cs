...
builder.Services.AddSingleton(SharedDomainDomainTypes.Generate(SharedDomainEventsJsonContext.Default.Options));
...
var apiRoute = app.MapGroup("/api").AddEndpointFilter<ExceptionEndpointFilter>();
...
apiRoute
    .MapPost(
        ...
        async ([FromBody] InputWeatherForecastCommand command, [FromServices] SekibanOrleansExecutor executor) =>
...
apiRoute
    .MapPost(
        ...
        async ([FromBody] RemoveWeatherForecastCommand command, [FromServices] SekibanOrleansExecutor executor) =>
        await executor.CommandAsync(command).ToSimpleCommandResponse().UnwrapBox())
