...
builder.Services.Configure<IOptions<ActorRuntimeOptions>>(options =>
{
...
var instanceId = Environment.GetEnvironmentVariable("CONTAINER_APP_REPLICA_NAME") ??
    Environment.GetEnvironmentVariable("HOSTNAME") ?? Environment.MachineName ?? Guid.NewGuid().ToString("N")[..8];

var consumerGroup = Environment.GetEnvironmentVariable("SEKIBAN_CONSUMER_GROUP") ??
    (app.Environment.IsDevelopment() ? "dapr-sample-projectors-dev" : "dapr-sample-projectors");
...
logger.LogInformation(
    "=== SEKIBAN API SERVICE CONFIGURED ({Environment} ENVIRONMENT) ===",
...
try
{
    var actorOptions = app.Services.GetService<IOptions<ActorRuntimeOptions>>();
...
startupLogger.LogInformation(
    ...
    Environment.GetEnvironmentVariable("DAPR_HTTP_PORT") ?? "Not Set (Default: 3500)");
startupLogger.LogInformation(
    "  - DAPR_GRPC_PORT: {DaprGrpcPort}",
    Environment.GetEnvironmentVariable("DAPR_GRPC_PORT") ?? "Not Set (Default: 50001)");
...
} else
{
    ...
    startupLogger.LogInformation(
        ...
        Environment.GetEnvironmentVariable("CONTAINER_APP_NAME") ?? "Not Set");
    startupLogger.LogInformation(
        "  - CONTAINER_APP_REPLICA_NAME: {ReplicaName}",
        Environment.GetEnvironmentVariable("CONTAINER_APP_REPLICA_NAME") ?? "Not Set");
    startupLogger.LogInformation(
        "  - CONTAINER_APP_REVISION: {Revision}",
        Environment.GetEnvironmentVariable("CONTAINER_APP_REVISION") ?? "Not Set");
}
...
try
{
    var testActorId = new ActorId("test-actor-id");
    var testProxy = app
        .Services
        .GetRequiredService<IActorProxyFactory>()
        .CreateActorProxy<IAggregateActor>(testActorId, nameof(AggregateActor));
...
app
    .MapPost(
        ...
        {
            logger.LogInformation(
                "=== CreateUser API called with UserId: {UserId}, Name: {Name} ===",
                command.UserId,
                command.Name);
            ...
            try
            {
                ...
                return result.IsSuccess
                    ? Results.Ok(
                        new { success = true, aggregateId = command.UserId, version = result.GetValue().Version })
...
app
    .MapPost(
        "/api/users/{userId}/update-name",
        async (Guid userId, UpdateUserNameRequest request, ISekibanExecutor executor) =>
        {
...
app
    .MapGet(
        "/api/users/{userId}",
        async (Guid userId, ISekibanExecutor executor, [FromServices] SekibanDomainTypes domainTypes) =>
        {
            var result = await executor.LoadAggregateAsync<UserProjector>(
...
app
    .MapGet(
        ...
        async (Guid userId, [FromQuery] string? waitForSortableUniqueId, [FromServices] ISekibanExecutor executor) =>
        {
...
app
    .MapPost(
        "/api/weatherforecast/input",
        async ([FromBody] InputWeatherForecastCommand command, [FromServices] ISekibanExecutor executor) =>
        {
...
app
    .MapPost(
        ...
        {
            logger.LogInformation(
                "=== UpdateWeatherForecastLocation API called with WeatherForecastId: {WeatherForecastId}, Location: {Location} ===",
                weatherForecastId,
                request.Location);
...
app
    .MapPost(
        "/api/weatherforecast/{weatherForecastId}/delete",
        async (Guid weatherForecastId, [FromServices] ISekibanExecutor executor) =>
        {
...
app
    .MapPost(
        "/api/weatherforecast/{weatherForecastId}/remove",
        async (
            Guid weatherForecastId,
            [FromServices] ISekibanExecutor executor,
            [FromServices] ILogger<Program> logger) =>
        {
            logger.LogInformation(
                "=== RemoveWeatherForecast API called with WeatherForecastId: {WeatherForecastId} ===",
                weatherForecastId);
...
app
    .MapGet(
        "/api/weatherforecast/{weatherForecastId}/aggregate-state",
        async (Guid weatherForecastId, [FromServices] ILogger<Program> logger) =>
        {
            try
            {
                logger.LogInformation(
                    "=== GetAggregateState API called with WeatherForecastId: {WeatherForecastId} ===",
                    weatherForecastId);
                ...
                foreach (var actorIdFormat in actorIdFormats)
                {
                    try
                    {
                        ...
                        if (response.IsSuccessStatusCode)
                        {
                            ...
                            logger.LogInformation(
                                "Success with actor ID: {ActorId}, Response: {Response}",
                                actorIdFormat,
                                content);
                        ...
                        logger.LogWarning(
                            "Failed with actor ID: {ActorId}, Status: {Status}",
                            actorIdFormat,
                            response.StatusCode);
                    }
                    catch (Exception innerEx)
            ...
            catch (Exception ex)
            {
                logger.LogError(
                    ex,
                    "Error getting aggregate state for WeatherForecastId: {WeatherForecastId}",
                    weatherForecastId);
                return Results.BadRequest(new { success = false, error = ex.Message });
            }
        })
...
app
    .MapGet(
        "/api/weatherforecast/{weatherForecastId}/version",
        async (
            Guid weatherForecastId,
            [FromServices] ISekibanExecutor executor,
            [FromServices] ILogger<Program> logger) =>
        {
            ...
            {
                logger.LogInformation(
                    "=== CheckAggregateVersion API called with WeatherForecastId: {WeatherForecastId} ===",
                    weatherForecastId);
            ...
            catch (Exception ex)
            {
                logger.LogError(
                    ex,
                    "Error checking aggregate version for WeatherForecastId: {WeatherForecastId}",
                    weatherForecastId);
                return Results.BadRequest(new { success = false, error = ex.Message });
            }
        })
...
app
    .MapPost(
        "/internal/pubsub/events/monitor",
        async (HttpContext context, [FromServices] ILogger<Program> logger) =>
        {
            ...
            var logEntry
                = $"[{lastEventTime:yyyy-MM-dd HH:mm:ss.fff}] Event #{eventCounter}: {body.Substring(0, Math.Min(200, body.Length))}...";
        ...
        () =>
        {
            return Results.Ok(
                new
                {
                    ...
                    lastEventTime = lastEventTime == DateTime.MinValue
                        ? "No events received"
                        : lastEventTime.ToString("yyyy-MM-dd HH:mm:ss.fff"),
                    timeSinceLastEvent = lastEventTime == DateTime.MinValue
                        ? "N/A"
                        : $"{(DateTime.UtcNow - lastEventTime).TotalSeconds:F1} seconds ago",
...
app
    .MapPost(
        "/api/test/pubsub-flow",
        async ([FromServices] ISekibanExecutor executor, [FromServices] ILogger<Program> testLogger) =>
        {
            ...
            return Results.Ok(
                new
                {
                    ...
                    testUserId = userId,
                    foundInProjection
                        = foundUser != null ? new { foundUser.UserId, foundUser.Name, foundUser.Email } : null,
