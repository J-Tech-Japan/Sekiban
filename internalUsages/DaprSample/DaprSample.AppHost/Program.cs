...
var api = builder
    .AddProject<DaprSample_Api>("dapr-sample-api")
...
var eventRelay = builder
    .AddProject<DaprSample_EventRelay>("dapr-sample-event-relay")
    .WithReference(postgres)
    .WaitFor(postgres)
    .WithDaprSidecar(
        new DaprSidecarOptions
...
builder.AddProject<DaprSample_Web>("dapr-sample-web").WithExternalHttpEndpoints().WithReference(api).WaitFor(api);
