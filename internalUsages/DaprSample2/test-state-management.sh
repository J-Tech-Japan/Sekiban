...
echo "1. Starting Placement service..."
/home/vboxuser/.dapr/bin/placement --port 50005 &
PLACEMENT_PID=$!
sleep 3

# Start Dapr with app
echo "2. Starting Dapr with application..."
dapr run \
  --app-id counter-demo \
  --app-port 5004 \
  --dapr-http-port 3501 \
...
echo "4. Waiting for actor runtime to initialize..."
sleep 5

# Test state management
echo "5. Testing state management..."
echo

# Get initial value
echo "   a) Getting initial counter value for 'test1':"
curl -s http://localhost:5004/counter/test1 || echo "Failed"
echo
...
echo "6. Testing different actor instance 'test2':"
echo "   Initial value:"
curl -s http://localhost:5004/counter/test2 || echo "Failed"
echo
