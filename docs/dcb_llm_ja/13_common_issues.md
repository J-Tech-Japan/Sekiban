...
- **(a) コミット前ロールバック** — コミット**前**に失敗（またはキャンセル）し、耐久クレームは存在しない。通常のプロバイダ失敗として、または権威的読み取りで勝者が見つからないキャンセルでは型付きリトライ可能 in-doubt として表出。**対処**: リトライ（キーをクレームして収束）。*証明*: SQLite / Postgres の「コミット前キャンセル」テスト（実DB）— 耐久物なし、リトライで append。
- **(b) 耐久コミット後の非キャンセル応答喪失** — トランザクションはコミットし、その後トランスポートエラーで応答喪失。**初回**は**元の**プロバイダ／トランスポート失敗を表出（型付き in-doubt でも成功でもない）。**対処**: 明示的リトライが権威的読み取り＋フィンガープリント（Cosmos ではコミット済み状態ゲート）を実行し、元の勝者レシートで `AlreadyCommittedSameOperation` に収束。*証明*: Postgres（実DB）・SQLite（実DB）・Cosmos（インメモリダブル上の本番フォールトシーム）のコミット後トランスポートテスト。
- **(c) 耐久コミット後のキャンセル／タイムアウト** — コミット成功後にキャンセル／タイムアウト。実装は同一呼び出し内で独立した読み戻しを行い、直接 `AlreadyCommittedSameOperation` に解決可能。**対処**: 多くは不要（同一呼び出しが既に勝者レシートを返す）。それ以外はリトライ。*証明*: SQLite「コミット後キャンセル」テスト、DynamoDB 曖昧キャンセルテスト（スレッドセーフ fake）。
- **(d) 権威的検証／読み取り不能** — 衝突／曖昧を解決できない（勝者を読めない、またはコミット済み状態を検証できない）。閉じた `Reason` を持つ型付きリトライ可能 `ConditionalAppendInDoubtException` として表出。**対処**: リトライ（勝者が読める／コミットされれば収束）。*証明*: Cosmos の裸409-勝者なし・修復失敗テスト、DynamoDB の裸衝突-勝者なしテスト。
