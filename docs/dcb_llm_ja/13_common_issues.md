...
**G19 による解決**。予約はアクターロック内・catch-up 後に、期待バージョンを **null/空を正規化した完全一致**として
比較します。空の期待バージョンは「タグが空であることを期待する」を意味し、2つ目の非オーバーラップな初回書き込みは
既存の `ResultBox.Error` チャネルで**衝突**します（新しい公開例外型なし）。
[初回書き込み予約のセマンティクス](03_aggregate_command_events.md#初回書き込み予約のセマンティクス-sek-g19)参照。
