...
**ただし、古い扉の周りの壁ではありません。** `Sekiban.Dcb.InMemory.InMemoryDcbExecutor` と旧ストア群は互換性のためランタイムパッケージ内に public のまま残り、既存コードは**これまでどおりコンパイルでき、動作します**。それが互換性の約束であり、意図的なものです。この経路に対する防御は `[Obsolete]` による誘導と、`AddSekibanDcbProductionGuard()` です。**ただしガードの答えは 2 種類あり、同じではありません。** インプロセスエグゼキューターは **Production では常に拒否**され、これを許可するオーバーライドは存在しません(どの名前で入手したかにも関わりません)。一方、**揮発ストアは既定では拒否されますが**、名前付きでストレージ専用の `AllowVolatileStorageInProduction` によって**運用者が明示的に許可できます**。自分が何かを申告しないストア(`Unknown`)は、いずれの場合も fail-closed のままです。

旧入口が閉じるのは次のメジャーです。それまで前に立ちはだかるのはコンパイラではなく**ガード**であり、テスト用エグゼキューターに対しては壁として、揮発ストレージに対しては「運用者が名前で下す判断」として立ちます。
