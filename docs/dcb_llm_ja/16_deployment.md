...
5 種類の生成テンプレートを同じバージョンで管理します。該当する依存グラフには `Microsoft.Extensions.*`
10.0.5 の下限と通常の Polly 推移依存があり、net9 と net10 は対応する 10.3.1 アセットを解決します。
10.0.1/10.3.1 が混在するローリングクラスタは検証済みサポートとはみなしません。
