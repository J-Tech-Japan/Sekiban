...
**並行性**。削除は **ETag ガード** 付きです。計画が固定した正確なバージョンを指定するため、運用者がレビューした後に行が書き換えられていれば Cosmos は削除を拒否し、移行も拒否します。再読み取りを行い、その行が「計画が削除可能な重複と判断したときと**まったく同じ内容**」である場合にのみ、新しいバージョンで再試行します。内容が少しでも変わっていれば、その行はそのまま残され、レースが報告されます(`LostRace`)。**移動した行に対して削除を強行するコード経路は存在しません。**
