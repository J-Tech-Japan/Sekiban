...
- **正規ペイロード（サポートする形）**: 生ペイロードを解決済みの型へデシリアライズしてドメインで再シリアライズし、正規 JSON として再出力します — **オブジェクトのキーは再帰的に序数ソート、配列要素の順序は保持、数値・文字列はドメインシリアライザの出力どおり**（Unicode エスケープとプロパティ順は影響せず、`1` と `1.0` は別物）。records/POCO を System.Text.Json（リフレクション/ソースジェネレータ）でシリアライズする場合に安定し、プロパティ宣言順に依存しません。シリアライズが**非決定的**なペイロード（順序なしコレクションを任意順の配列で出力する、非決定的な出力を行うコンバータ等）は条件付き追記の**サポート対象外**です。決定的な表現を使ってください。デシリアライズ/正規化できないペイロードはフェイルクローズします。
