...
1. **フェーズ 1 — 生の識別** (`SerializedCommitVersionDiscriminator`): `version` プロパティを、型付きペイロードの
   ...
   - `version` なし → レガシー経路。
   - 整数 `version` が 1 → 既知バージョン。
   - 整数 `version` が 1 以外 → **`UnsupportedSerializedCommitEnvelopeVersionException`** (副作用の前に fail closed)。
   - 非オブジェクトのルート、非整数 `version`、`version` の重複 → **`MalformedSerializedCommitException`** (別種の型付き
     shape エラー)。
