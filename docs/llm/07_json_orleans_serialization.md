...
Sekiban leverages Orleans' serialization system for distributed messaging and storage. Orleans requires all types that are passed between grains to be serializable.
...
If you need to customize Orleans serialization, Sekiban provides a way to use Newtonsoft.Json instead of the default System.Text.Json:
...
1. **Use Records for DTOs**: Use C# records for all Data Transfer Objects (DTOs) to ensure immutability and simplified serialization
