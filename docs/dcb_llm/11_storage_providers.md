...
- **Canonical payload (supported shapes)**: the raw payload is deserialized into the resolved type and re-serialized by the domain, then re-emitted as canonical JSON — **object keys recursively ordinal-sorted, array element order preserved, numbers and strings as the domain serializer emits them** (Unicode escaping and property order therefore do not matter; `1` and `1.0` are distinct). This is stable for records/POCOs serialized with System.Text.Json (reflection or source-gen), independent of property declaration order. Payloads whose serialization is **not deterministic** — e.g. unordered collections emitted as arrays in arbitrary order, or converters that emit non-deterministic output — are **not supported** for conditional append; use a deterministic representation. A payload that cannot be deserialized/canonicalized fails closed.
