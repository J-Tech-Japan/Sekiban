...
- **(a) Rollback before commit** — the write failed (or was cancelled) BEFORE it committed; no durable claim exists. Surfaces as the normal provider failure, or, for a cancellation resolved by an authoritative read that finds no winner, a typed retryable in-doubt. **Recipe**: retry; it converges by claiming the key. *Proven by*: SQLite and Postgres "cancelled-before-commit" tests (real DB) — nothing durable, retry appends.
- **(b) Non-cancellation response loss after a durable commit** — the transaction committed, then the response was lost via a transport error. The FIRST call surfaces the ORIGINAL provider/transport failure (not a typed in-doubt, not a success). **Recipe**: an explicit retry performs the authoritative read + fingerprint (and, on Cosmos, the committed-state gate) and converges to `AlreadyCommittedSameOperation` with the original winner's receipt. *Proven by*: Postgres (real DB), SQLite (real DB), Cosmos (production fault seam over an in-memory double) post-commit transport tests.
- **(c) Cancellation/timeout after a durable commit** — the commit succeeded, then the call was cancelled/timed out. The implementation performs a same-call independent read-back and can resolve directly to `AlreadyCommittedSameOperation`. **Recipe**: usually nothing — the same call already returned the winner's receipt; otherwise retry. *Proven by*: SQLite "cancellation-after-commit" test; DynamoDB ambiguous-cancellation test (thread-safe fake).
- **(d) Authoritative verification/read unavailable** — a conflict or ambiguity could not be resolved because the winner could not be read (or its committed state could not be verified). Surfaces as typed retryable `ConditionalAppendInDoubtException` with a closed `Reason`. **Recipe**: retry; it converges once the winner is readable/committed. *Proven by*: Cosmos bare-409-no-winner and repair-failure tests; DynamoDB bare-conflict-no-winner test.
