...
[GenerateSerializer]
public record AddItemToShoppingCart(Guid ShoppingCartId, string Name, int Quantity, Guid ItemId, int Price)
    : ICommandWithHandlerAsync<AddItemToShoppingCart, ShoppingCartProjector>
{
    ...
    public Task<ResultBox<EventOrNone>> HandleAsync(
        ...
        EventOrNone
            .Event(new ShoppingCartItemAdded(command.Name, command.Quantity, command.ItemId, command.Price))
            .ToTask();
}