...
public class SerializableMultiProjectionStateTests
{
    ...
    public async Task SerializeDeserialize_MultiProjectorPayload_Success()
    {
        ...
        // Act
        var serializable = await SerializableMultiProjectionState.CreateFromAsync(originalState, _domainTypes);
    ...
    public async Task SerializeDeserialize_AggregateListProjector_Success()
    {
        ...
        foreach (var partitionKeys in originalPayload.Aggregates.Keys)
        {
            ...
            Assert.Equal(
                originalAggregate.PartitionKeys.RootPartitionKey,
                restoredAggregate.PartitionKeys.RootPartitionKey);
    ...
    public async Task SerializeDeserialize_LargeData_Success()
    {
        ...
        for (var i = 0; i < 100; i++)
        {
            ...
            users = users.Add(
                userId,
                new MultiProjectorPayload.User(userId, $"User{i}", $"user{i}@example.com", i % 2 == 0));
            ...
            {
                items = items.Add(
                    new MultiProjectorPayload.Item(Guid.NewGuid(), $"Item-{i}-{j}", j + 1, (j + 1) * 10.0m));
            }
        ...
        {
            Assert.True(restoredPayload.Users.ContainsKey(userId));
            Assert.Equal(originalPayload.Users[userId].Name, restoredPayload.Users[userId].Name);
        }

        foreach (var cartId in originalPayload.Carts.Keys.Take(10))
        {
            Assert.True(restoredPayload.Carts.ContainsKey(cartId));
            Assert.Equal(originalPayload.Carts[cartId].Items.Count, restoredPayload.Carts[cartId].Items.Count);
        }
    }
    ...
    private MultiProjectorPayload CreateTestMultiProjectorPayload()
    {
        ...
        var users = ImmutableDictionary<Guid, MultiProjectorPayload.User>
            .Empty
        ...
        var cart1Items = ImmutableList<MultiProjectorPayload.Item>
            .Empty
        ...
        var cart2Items
            = ImmutableList<MultiProjectorPayload.Item>.Empty.Add(
                new MultiProjectorPayload.Item(Guid.NewGuid(), "Item3", 3, 30.0m));

        var carts = ImmutableDictionary<Guid, MultiProjectorPayload.Cart>
            .Empty
    ...
    private AggregateListProjector<BranchProjector> CreateTestAggregateListProjector()
    {
        ...
        var aggregates = ImmutableDictionary<PartitionKeys, Aggregate>
            .Empty
    ...
    private MultiProjectionState CreateMultiProjectionState<TProjection>(TProjection projector)
        ...
        new(projector, Guid.NewGuid(), "20250331T123456Z", 42, 21, "test-root-partition-key");

    /// <summary>
    ///     Compares the contents of MultiProjectionState
    /// </summary>
    private void AssertMultiProjectionStatesEqual(MultiProjectionState expected, MultiProjectionState actual)
    {
        ...
        Assert.Equal(expected.ProjectorCommon.GetType().FullName, actual.ProjectorCommon.GetType().FullName);
    }
    ...
    private class PartitionKeysJsonConverter : JsonConverter<PartitionKeys>
    {
        public override PartitionKeys Read(ref Utf8JsonReader reader, Type typeToConvert, JsonSerializerOptions options)
        {
            ...
            var rootPartitionKey = string.Empty;
