...
public class VersionIncrementTest : IAsyncLifetime
{
    ...
    public VersionIncrementTest(ITestOutputHelper output) => _output = output;
    ...
    public async Task UpdateUserName_MultipleNoOpUpdates_ShouldNotIncrementVersion()
    {
        ...
        for (var i = 0; i < 5; i++)
        {
            ...
            _output.WriteLine(
                $"Update {i + 1}: Version = {updateResult.GetValue().Version}, Events = {updateResult.GetValue().Events.Count}");
