...
public class AggregateEventHandlerActor : Actor, IAggregateEventHandlerActor
{
        ...
        try
        {
            ...
            foreach (var eventDoc in newEventDocuments)
            {
                try
                {
                    ...
                    {
                        _logger.LogWarning(
                            "Failed to convert SerializableEventDocument to event for type: {PayloadTypeName}",
            ...
            var existingEventDocs
                = await StateManager.TryGetStateAsync<List<SerializableEventDocument>>(EventDocumentsStateKey);
            var eventDocsList = existingEventDocs.HasValue
                ? existingEventDocs.Value
                : new List<SerializableEventDocument>();
            ...
            var lastEventId = newEventDocuments.Any()
                ? newEventDocuments.Last().SortableUniqueId
    ...
    public async Task<List<SerializableEventDocument>> GetDeltaEventsAsync(string fromSortableUniqueId, int limit)
    {
        try
        {
            ...
            if (storedEventDocs.HasValue && storedEventDocs.Value.Any())
            {
                ...
                {
                    return limit > 0 ? allEventDocs.Take(limit).ToList() : allEventDocs;
                }
                ...
                return limit > 0 ? deltaEventDocs.Take(limit).ToList() : deltaEventDocs.ToList();
            }
            ...
            try
            {
                ...
                {
                    _logger.LogWarning(
                        "Failed to retrieve events from external storage: {Error}",
                ...
                if (!string.IsNullOrWhiteSpace(fromSortableUniqueId))
                {
                    filteredEvents = filteredEvents.SkipWhile(e => e.SortableUniqueId != fromSortableUniqueId).Skip(1);
                }
    ...
    /// </summary>
    public Task RegisterProjectorAsync(string projectorKey) => Task.CompletedTask;

    private async Task<List<SerializableEventDocument>> ConvertEventsToSerializableDocuments(
    ...
    private EventRetrievalInfo GetEventRetrievalInfo()
    {
        ...
        var partitionKeysString = actorId.Contains(':') ? actorId.Substring(actorId.IndexOf(':') + 1) : actorId;
