...
public record SerializableListQuery
{
    ...
    private SerializableListQuery(string queryTypeName, byte[] compressedQueryJson, string queryAssemblyVersion)
    {
    ...
    public async Task<ResultBox<IListQueryCommon>> ToListQueryAsync(SekibanDomainTypes domainTypes)
    {
        try
        {
            Type? queryType = null;
            ...
            if (CompressedQueryJson.Length > 0)
            {
                ...
                catch (NotSupportedException)
                {
                    ...
                    query = JsonSerializer.Deserialize(decompressedJson, queryType) as IListQueryCommon;
                }
