...
public class DaprEventStore : Repository, IEventWriter, IEventReader
{
    ...
    public DaprEventStore(DaprClient daprClient, IOptions<DaprSekibanOptions> options, ILogger<DaprEventStore> logger)
    {
        ...
        Deserializer = json =>
            JsonSerializer.Deserialize<IEvent>(json) ??
    ...
    public async Task<ResultBox<IReadOnlyList<IEvent>>> GetEvents(EventRetrievalInfo retrievalInfo)
    {
        try
        {
            ...
            if (retrievalInfo.GetIsPartition() && retrievalInfo.HasAggregateStream())
            {
                ...
                events = Events.Where(e => e.PartitionKeys == partitionKeys).OrderBy(e => e.SortableUniqueId).ToList();
            } else
            {
                // Return all events if no specific partition requested
                events = Events.OrderBy(e => e.SortableUniqueId).ToList();
            }
    ...
    public async Task SaveEvents<TEvent>(IEnumerable<TEvent> events) where TEvent : IEvent
    {
        try
        {
            ...
            foreach (var @event in eventList)
            {
                ...
                await _daprClient.SaveStateAsync(_options.StateStoreName, key, @event);
            }

            // Publish events
            foreach (var @event in eventList)
            {
                await _daprClient.PublishEventAsync(_options.PubSubName, _options.EventTopicName, @event);
            }
        }
        catch (Exception ex)
        {
    ...
    public async new Task<ResultBox<List<IEvent>>> Save(List<IEvent> events)
    {
        ...
        try
        {
            ...
            await _daprClient.SaveStateAsync(_options.StateStoreName, "all-events", allEvents);
