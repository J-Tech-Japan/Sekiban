...
public sealed class AzureBlobDuckDbColdObjectStorage : IColdObjectStorage
{
    private readonly BlobContainerClient _container;
    private readonly string _blobName;
    private readonly SemaphoreSlim _mutex = new(1, 1);

    public AzureBlobDuckDbColdObjectStorage(
    ...
    public async Task<ResultBox<ColdStorageObject>> GetAsync(string path, CancellationToken ct)
    {
        ...
        try
        {
            ...
            var inner = new DuckDbColdObjectStorage(state.LocalPath);
    ...
    public async Task<ResultBox<bool>> PutAsync(string path, byte[] data, string? expectedETag, CancellationToken ct)
    {
        ...
        catch (RequestFailedException ex) when (ex.Status == 412)
        {
            return ResultBox.Error<bool>(new InvalidOperationException(
                "Concurrent update detected while writing duckdb cold storage blob", ex));
        }
        catch (Exception ex)
        {
    ...
    public async Task<ResultBox<bool>> DeleteAsync(string path, CancellationToken ct)
    {
        ...
        {
            return ResultBox.Error<bool>(new InvalidOperationException(
                "Concurrent update detected while deleting from duckdb cold storage blob", ex));
        }
        catch (Exception ex)
        {
            return ResultBox.Error<bool>(ex);
    ...
    {
        var localPath = Path.Combine(Path.GetTempPath(), $"sekiban-cold-duckdb-{Guid.NewGuid():N}.db");
