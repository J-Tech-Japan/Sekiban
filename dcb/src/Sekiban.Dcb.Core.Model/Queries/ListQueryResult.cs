    ...
    IEnumerable<T> Items) where T : notnull
{
    ...
    /// </summary>
    public static ListQueryResult<T> CreatePaginated(IQueryPagingParameter pagingParam, List<T> allItems)
    {
        if (pagingParam.PageNumber == null || pagingParam.PageSize == null)
        {
            // Return all items without pagination
            return new ListQueryResult<T>(allItems.Count, null, null, null, allItems);
        }
        ...
        if (pageNumber < 1 || pageNumber > totalPages)
        {
            ...
            return new ListQueryResult<T>(totalCount, totalPages, pageNumber, pageSize, Array.Empty<T>());
        }

        var pagedItems = allItems.Skip((pageNumber - 1) * pageSize).Take(pageSize);

        return new ListQueryResult<T>(totalCount, totalPages, pageNumber, pageSize, pagedItems);
    }
