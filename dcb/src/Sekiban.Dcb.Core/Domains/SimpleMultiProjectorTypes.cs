...
internal class SimpleMultiProjectorTypes : ICoreMultiProjectorTypes
{
    ...
    /// </summary>
    public void RegisterProjector<TProjector>() where TProjector : ICoreMultiProjector<TProjector>, new()
    {
    ...
    ///     Register a multi projector type using its static GetMultiProjectorName
    /// </summary>
    public void RegisterProjectorWithoutResult<TProjector>() where TProjector : ICoreMultiProjector<TProjector>, new()
    {
        var projectorName = TProjector.MultiProjectorName;
        ...
        IMultiProjectionPayload payload)
    {
        try
        {
            ...
            // Fallback: JSON serialize + auto Gzip compression
            // Custom serializers control their own compression; non-custom gets auto Gzip
            var json = JsonSerializer.Serialize(payload, payload.GetType(), domainTypes.JsonSerializerOptions);
            var compressed = GzipCompression.CompressString(json);
