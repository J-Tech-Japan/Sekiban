    ...
    where TTagProjector : ITagProjector<TTagProjector> where TTagGroup : IGuidTagGroup<TTagGroup>
{
    ...
    /// </summary>
    private static ResultBox<GenericTagMultiProjector<TTagProjector, TTagGroup>> ProjectInternal(
    ...
    /// </summary>
    public static GenericTagMultiProjector<TTagProjector, TTagGroup> Project(
        GenericTagMultiProjector<TTagProjector, TTagGroup> payload,
        Event ev,
        List<ITag> tags,
        DcbDomainTypes domainTypes,
        SortableUniqueId safeWindowThreshold)
    {
        ...
        return GuardedUnwrap.Unwrap(
            result,
            new BoundaryContext("GenericTagMultiProjector.Project", ev?.EventType));
    }
        ...
        DcbDomainTypes domainTypes)
    {
        ...
        var unsafeEventIdsBefore = State.GetAllUnsafeEvents().Select(e => e.Id).ToHashSet();
        var projected = Project(this, evt, tags, domainTypes, safeWindowThreshold);
        var promotedUnsafeEvents = projected.State
            .GetAllUnsafeEvents()
            .Where(e => unsafeEventIdsBefore.Contains(e.Id))
            .ToHashSet();
        var promotedToSafe = State.GetAllUnsafeEvents()
            .Where(e => !promotedUnsafeEvents.Contains(e))
            .ToList();
