...
/// <summary>
///     In-memory wrapper that exposes the exception-based <see cref="ISekibanExecutorWithoutResult"/> contract.
///     Internally delegates to an <see cref="ISekibanExecutor"/> instance (defaulting to <see cref="InMemoryDcbExecutor"/>)
///     and unwraps <see cref="ResultBox{T}"/> results using <see cref="ResultBoxExtensions.UnwrapBox"/>.
/// </summary>
public class InMemoryDcbExecutorWithoutResult : ISekibanExecutorWithoutResult
{
    private readonly ISekibanExecutor _executor;

    public InMemoryDcbExecutorWithoutResult(DcbDomainTypes domainTypes)
        : this(new InMemoryDcbExecutor(domainTypes)) { }

    public InMemoryDcbExecutorWithoutResult(InMemoryDcbExecutor executor)
        : this((ISekibanExecutor)executor) { }

    public InMemoryDcbExecutorWithoutResult(ISekibanExecutor executor)
    {
