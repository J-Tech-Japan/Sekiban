...
public record SafeUnsafeProjectionStateV3<T> where T : class
{
    ...
    public SafeUnsafeProjectionStateV3<T> ProcessEvent<TPayload>(
        Event evt,
        Func<TPayload, IEnumerable<ProjectionRequest<T>>> requestHandler) 
    ...
    private SafeUnsafeProjectionStateV3<T> ProcessNewlySafeEvents()
    {
        ...
        foreach (var kvp in _safeBackup)
        {
            ...
            var stillUnsafeEvents = backup.UnsafeEvents
                .Where(e => string.Compare(e.SortableUniqueIdValue, SafeWindowThreshold, StringComparison.Ordinal) > 0)
                .ToList();
        ...
        var state = new SafeUnsafeProjectionStateV3<T>(
            newCurrentData,
            newSafeBackup,
            SafeWindowThreshold
        );
    ...
    /// </summary>
    private SafeUnsafeProjectionStateV3<T> ProcessSafeProjection(
        List<ProjectionRequest<T>> requests,
        Event evt)
    {
        var newCurrentData = new Dictionary<Guid, T>(_currentData);
        ...
        return new SafeUnsafeProjectionStateV3<T>(
            newCurrentData,
            newSafeBackup,
            SafeWindowThreshold
        );
    ...
    /// </summary>
    private SafeUnsafeProjectionStateV3<T> ProcessUnsafeProjection(
        List<ProjectionRequest<T>> requests,
        Event evt)
    {
        var newCurrentData = new Dictionary<Guid, T>(_currentData);
    ...
    {
        return _safeBackup.TryGetValue(itemId, out var backup) 
            ? backup.UnsafeEvents 
    ...
    {
        return _safeBackup.Values
            .SelectMany(b => b.UnsafeEvents)
            .Distinct()
            .OrderBy(e => e.SortableUniqueIdValue);
    }
    ...
    public IEnumerable<(Guid ItemId, T SafeState, IEnumerable<Event> EventsToReprocess)> GetItemsForReprocessing()
    {
        foreach (var kvp in _safeBackup)
        {
            var eventsNowSafe = kvp.Value.UnsafeEvents
                .Where(e => string.Compare(e.SortableUniqueIdValue, SafeWindowThreshold, StringComparison.Ordinal) <= 0)
                .OrderBy(e => e.SortableUniqueIdValue);
