...
public static class NameValidator
{
    ...
    public static void ValidateTagGroupNameAndThrow(string tagGroupName)
    {
        ...
        {
            throw new ArgumentException($"Tag group name '{tagGroupName}' contains invalid characters. Colon (:) is not allowed as it is used as a delimiter");
        }
        
        // Create a dummy tag to validate the group name using existing validation
        var dummyTag = new DummyTag(tagGroupName, "dummy");
        var errors = Tags.TagValidator.ValidateTag(dummyTag);
        
        // Filter for group-related errors only
        var groupErrors = errors.Where(e => 
            e.ErrorType == Tags.TagValidationErrorType.EmptyGroup ||
            e.ErrorType == Tags.TagValidationErrorType.InvalidCharactersInGroup ||
            e.ErrorType == Tags.TagValidationErrorType.GroupTooLong).ToList();
    ...
    private class DummyTag : Tags.ITag
    {
