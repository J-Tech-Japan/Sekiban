...
{
    public static Task<ResultBox<string>> SerializeAggregateList<TAggregateProjector>(AggregateListProjector<TAggregateProjector> projector, SekibanDomainTypes domainTypes) where TAggregateProjector : IAggregateProjector, new()=>
        ResultBox.WrapTry(
            async () =>
            {
    ...
    public static Task<ResultBox<AggregateListProjector<TAggregateProjector>>> DeserializeAggregateList<TAggregateProjector>(
        string serialized,
        ...
        ResultBox.WrapTry(
            async () =>
            {
                ...
                foreach (var aggregate in serializable.List)
                {
                    ...
                    if (deserialized.HasValue) dictionary.Add(deserialized.GetValue().PartitionKeys,deserialized.GetValue());
                }
