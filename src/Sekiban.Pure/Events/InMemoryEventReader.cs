...
public class InMemoryEventReader(Repository repository) : IEventReader
{
    ...
    public async Task<ResultBox<IReadOnlyList<IEvent>>> GetEvents(EventRetrievalInfo eventRetrievalInfo)
    {
        ...
        {
            SinceSortableIdCondition since => query.Where(m =>
                m.SortableUniqueId.CompareTo(since.SortableUniqueId.Value) > 0),
            BetweenSortableIdCondition between => query.Where(m =>
                m.SortableUniqueId.CompareTo(between.Start.Value) > 0 &&
