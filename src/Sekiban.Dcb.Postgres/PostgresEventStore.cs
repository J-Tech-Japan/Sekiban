...
public class PostgresEventStore : IEventStore
{
    ...
    public async Task<ResultBox<IEnumerable<Event>>> ReadEventsByTagAsync(ITag tag, SortableUniqueId? since = null)
    {
        try
        {
            ...
            var query = context.Events
                .Where(e => EF.Functions.JsonContains(e.Tags, tagJson));
    ...
    {
        return _domainTypes.EventTypes.SerializeEventPayload(payload);
    }
    ...
    private ResultBox<IEventPayload> DeserializeEventPayload(string eventType, string json)
    {
        try
        {
            ...
            if (payload == null)
            {
                return ResultBox.Error<IEventPayload>(new Exception($"Failed to deserialize event payload of type {eventType}. Make sure the event type is registered."));
            }
