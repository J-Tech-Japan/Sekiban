...
public record SortableUniqueIdValue(string Value)
{
    ...
    public static implicit operator SortableUniqueIdValue(string v) => new SortableUniqueIdValue(v);
    ...
    public static string GetSafeIdFromUtc() => SekibanDateProducer.GetRegistered().UtcNow.AddMilliseconds(-safeMilliseconds).Ticks +
        (Math.Abs(Guid.Empty.GetHashCode()) % 1000000000000).ToString("000000000000");
    public string GetSafeId() => GetTicks().AddSeconds(-safeMilliseconds).Ticks +
        (Math.Abs(Guid.Empty.GetHashCode()) % 1000000000000).ToString("000000000000");
