    ...
    where TAggregatePayload : IAggregatePayloadCommon
{
    ...
    public IAggregateTestHelper<TAggregatePayload> GivenEnvironmentEventsFile(string filename) =>
        GivenEnvironmentEventsFile(filename, false);
    ...
    public List<IEvent> GetAllAggregateEvents(int? toVersion = null)
    {
        ...
        return aggregateLoader
                .AllEventsAsync<TAggregatePayload>(GetAggregateId(), GetRootPartitionKey(), toVersion)
                .Result
                ?.ToList() ??
            [];
    }
    ...
    private IAggregateTestHelper<TAggregatePayload>
        WhenCommandPrivate<TAggregatePayloadIn, TCommand>(TCommand command, bool withPublish)
        where TAggregatePayloadIn : IAggregatePayloadCommon where TCommand : ICommandCommon<TAggregatePayloadIn> =>
    ...
    private IAggregateTestHelper<TAggregatePayload> WhenCommandPrivateFunc<TAggregatePayloadIn, TCommand>(
        Func<AggregateState<TAggregatePayload>, TCommand> commandFunc,
        bool withPublish) where TAggregatePayloadIn : IAggregatePayloadCommon
        where TCommand : ICommandCommon<TAggregatePayloadIn>
    {
        ResetBeforeCommand();
        ...
        if (command is ICommandConverterCommon converter)
        {
            ...
            if (((dynamic)handler).ConvertCommand((dynamic)converter) is ICommandCommon convertedCommand)
            {
                ...
                {
                    return generated.Invoke(this, new object?[] { convertedCommand, withPublish }) as
                            IAggregateTestHelper<TAggregatePayload> ??
                        throw new SekibanTypeNotFoundException("Failed to get result of WhenCommandPrivate method");
                }
            }
        ...
        try
        {
            if (command is ICommandWithoutLoadingAggregateCommon &&
                command is not ICommandWithHandlerCommon<TAggregatePayloadIn, TCommand>)
            {
                var handler
                    = _serviceProvider.GetService(typeof(ICommandHandlerCommon<TAggregatePayloadIn, TCommand>)) as
                        ICommandHandlerCommon<TAggregatePayloadIn, TCommand> ??
                ...
                var commandResponse
                    = (CommandResponse)((dynamic?)method.Invoke(
                        ...
                        throw new SekibanCommandHandlerNotMatchException(
                            "Command failed to execute " + command.GetType().Name)).Result;
                _latestEvents = [.. commandResponse.Events];
            } else if (command is ICommandWithoutLoadingAggregateCommon &&
                command is ICommandWithHandlerCommon<TAggregatePayloadIn, TCommand>)
            {
            ...
            } else if (command is ICommandWithHandlerCommon<TAggregatePayloadIn, TCommand>)
            {
                ...
                var parent = typeof(TAggregatePayloadIn).GetBaseAggregatePayloadTypeFromAggregate();
    ...
    public IAggregateTestHelper<TAggregatePayload> ThenSingleProjectionPayloadIsFromJson<TSingleProjectionPayload>(
        string payloadJson) where TSingleProjectionPayload : class, ISingleProjectionPayloadCommon
    {
        ThenNotThrowsAnException();
        var actual = GetSingleProjectionState<TSingleProjectionPayload>().Payload;
