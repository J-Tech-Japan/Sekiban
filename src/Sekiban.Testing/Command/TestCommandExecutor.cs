...
public class TestCommandExecutor
{
    ...
    public Guid ExecuteCommand<TAggregatePayload>(ICommand<TAggregatePayload> command,
    ...
    public Guid ExecuteCommandWithPublish<TAggregatePayload>(ICommand<TAggregatePayload> command,
        Guid? injectingAggregateId = null)
        where TAggregatePayload : IAggregatePayload, new()
    {
    ...
    private Guid ExecuteCommand<TAggregatePayload>(ICommand<TAggregatePayload> command, Guid? injectingAggregateId,
        bool withPublish)
        where TAggregatePayload : IAggregatePayload, new()
    {
        var validationResults = command.ValidateProperties().ToList();
        ...
        if (command is not IOnlyPublishingCommandCommon)
        {
            var commandDocumentBaseType = typeof(CommandDocument<>);
            var commandDocumentType = commandDocumentBaseType.MakeGenericType(command.GetType());
            var commandDocument = Activator.CreateInstance(commandDocumentType, aggregateId, command,
                typeof(TAggregatePayload), null);
            ...
            LatestEvents = (ImmutableList<IEvent>)commandResponse.Events;
        }
        else
        {
            var commandDocumentBaseType = typeof(CommandDocument<>);
