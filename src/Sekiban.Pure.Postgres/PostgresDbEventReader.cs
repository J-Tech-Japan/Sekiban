...
public class PostgresDbEventReader : IEventReader
{
    ...
    {
        return await _dbFactory.DbActionAsync(async dbContext =>
        {
            ...
            query = eventRetrievalInfo.SortableIdCondition switch
            {
                ...
                BetweenSortableIdCondition between => query
                    .Where(e => string.Compare(e.SortableUniqueId, between.Start.Value) > 0 &&
            ...
            foreach (var dbEvent in dbEvents)
            {
                ...
                var converted = _eventTypes.DeserializeToTyped(eventDocument, _serializer.GetJsonSerializerOptions());
